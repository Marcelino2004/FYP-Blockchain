...
export const usePlatformStats = () => {
  ...
  const fetchStats = useCallback(async () => {
    ...
    try {
      // ONLY try contract if lendingPoolLens exists and has getAddress method
      if (
        contracts.lendingPoolLens &&
        typeof contracts.lendingPoolLens.getAddress === "function"
      ) {
        console.log("📊 Fetching platform stats from contract...");
        const address = await contracts.lendingPoolLens.getAddress();
        console.log("🔍 LendingPoolLens address:", address);
        console.log("🔍 Contract object:", contracts.lendingPoolLens);

        // Check if contract has the function
        if (typeof contracts.lendingPoolLens.getPlatformStats !== "function") {
          throw new Error("getPlatformStats function not found on contract");
        }
