...
const CONFIG = {
  ...
  tokenAddresses: {
    sepolia: {
      WETH: "0xd1A107f9962317667B23D49fA35ac53A5Be98fAe", // 0x7b79995e5f793A07Bc00c21412e50Ecae098E7f9
...
async function main() {
  ...
  // Relax oracle staleness for testnet
  await priceOracle.setStalePriceThreshold(7 * 24 * 60 * 60);
  console.log("   ✅ Oracle stale price threshold updated");
  ...
  const ORACLE_FEED_KEY = {
    WETH: "ETH_USD",
    WBTC: "BTC_USD",
    USDC: "USDC_USD",
  };

  for (const symbol of Object.keys(ORACLE_FEED_KEY)) {
    const tokenAddr = tokenAddresses[symbol];
    const feedAddr = priceFeeds[ORACLE_FEED_KEY[symbol]];

    if (!tokenAddr || !feedAddr) {
      console.log(`   ⚠️  Skipping ${symbol} (missing address or feed)`);
      continue;
    }

    try {
      await priceOracle.setPriceFeed(tokenAddr, feedAddr, symbol);

      const supported = await priceOracle.isSupportedToken(tokenAddr);
      console.log(`   ✅ ${symbol} price feed set (supported=${supported})`);
    } catch (err) {
      console.log(`   ❌ ${symbol} price feed failed: ${err.message}`);
    }
  }
  ...
  await collateralManager.grantRole(LENDING_POOL_ROLE_CM, lendingPoolAddress);
  ...
  for (const token of CONFIG.collateralTokens) {
    ...
    if (!tokenAddress) {
      console.log(`   ⚠️  ${token.name} address not found, skipping`);
      continue;
    }

    // Hard guard: oracle must already support token
    const isSupported = await priceOracle.isSupportedToken(tokenAddress);
    if (!isSupported) {
      console.log(
        `   ❌ ${token.name} NOT supported by oracle — skipping collateral add`
      );
      continue;
    }
    ...
    try {
      ...
      console.log(`   ❌ ${token.name} collateral add failed: ${err.message}`);
    }
  }
  ...
  await reputationManager.grantRole(LENDING_POOL_ROLE_RM, lendingPoolAddress);
  console.log("   ✅ LENDING_POOL_ROLE granted to LendingPool");
